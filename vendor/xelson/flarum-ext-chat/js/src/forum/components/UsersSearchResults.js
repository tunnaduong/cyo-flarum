...
export default class UsersSearchResults {
  ...
  view(query) {
    ...
    const usersList = (this.results[query] || [])
      .concat(app.store.all('users').filter((user) => [user.username(), user.displayName()].some((value) => value.toLowerCase().includes(query))))
    ...
    if (!resultsFind.length) {
      if (!this.searching) return <li className="SearchFailed">{app.translator.trans('xelson-chat.forum.chat.list.add_modal.search.failed')}</li>;
