...
export default class ChatViewport extends Component {
  ...
  getChatWrapper() {
    return app.screen() === 'phone' && app.current.matches(ChatPage) ? document.documentElement : document.querySelector('.ChatViewport .wrapper');
  }
  ...
  wrapperOnCreate(vnode) {
    ...
    (app.current.matches(ChatPage) ? window : vnode.dom).addEventListener('scroll', (this.boundScrollListener = this.wrapperOnScroll.bind(this)), {
      passive: true,
    });
  }
  ...
  scrollToBottom(force = false) {
    ...
    if (chatWrapper) {
      ...
      const fewMessages =
        app.current.matches(ChatPage) && document.querySelector('.ChatViewport .wrapper').scrollHeight + 200 < document.documentElement.clientHeight;
